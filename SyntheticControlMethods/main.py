...
class DataProcessor(object):
    ...
    def _process_input_data(
        self, dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, pen, exclude_columns, random_seed, **kwargs
    ):
        """
        ...
        treated_outcome_all, treated_outcome, unscaled_treated_covariates = self._process_treated_data(
            dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, periods_all, periods_pre_treatment, covariates, n_covariates
        )

        ### Now for control unit matrices ###
        control_outcome_all, control_outcome, unscaled_control_covariates = self._process_control_data(
            dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, n_controls, periods_all, periods_pre_treatment, covariates
        )

        # Rescale covariates to be unit variance (helps with optimization)
        treated_covariates, control_covariates = self._rescale_covariate_variance(
            unscaled_treated_covariates, unscaled_control_covariates, n_covariates
        )

        # Get matrix of unitwise differences between control units to treated unit
        pairwise_difference = self._get_pairwise_difference_matrix(treated_covariates, control_covariates)
    ...
    def _process_treated_data(
        self, dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, periods_all, periods_pre_treatment, covariates, n_covariates
    ):
        """
    ...
    def _process_control_data(
        self, dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, n_controls, periods_all, periods_pre_treatment, covariates
    ):
        """
...
class Synth(DataProcessor, Optimize, Plot, Tables, ValidityTests):
    ...
    def __init__(
        self, dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, n_optim=10, pen=0, exclude_columns=[], random_seed=0, **kwargs
    ):
        """
        data:
        ...
        original_checked_input = self._process_input_data(
            dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, pen, exclude_columns, random_seed, **kwargs
        )
        ...
        self.optimize(
            ...
            self.original_data.treated_covariates,
            self.original_data.control_outcome,
            self.original_data.control_covariates,
...
class DiffSynth(DataProcessor, Optimize, Plot, Tables, ValidityTests):
    ...
    def __init__(
        ...
        **kwargs
        ...
        modified_checked_input = self._process_input_data(
            modified_dataset, outcome_var, id_var, time_var, treatment_period, treated_unit, pen, exclude_columns, random_seed, **kwargs
        )
        ...
        # Get synthetic Control
        self.optimize(
            self.modified_data.treated_outcome,
            self.modified_data.treated_covariates,
            self.modified_data.control_outcome,
            self.modified_data.control_covariates,
    ...
    def difference_data(self, dataset, not_diff_cols):
        ...
        modified_dataset[data.outcome_var] = (
            modified_dataset.groupby(data.id)[data.outcome_var].apply(lambda unit: unit.interpolate(method="linear", limit_direction="both")).diff()
        )
