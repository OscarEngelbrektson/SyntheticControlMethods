...
class Tables(object):
    ...
    def _get_weight_df(self, data):
        ...
        weight_df = pd.DataFrame({"Weight": data.w.T[0]}, index=data.control_units)
    ...
    def _get_comparison_df(self, data):
        ...
        wmape = (np.abs((unscaled_treated_covariates - unscaled_control_covariates)) @ data.w).reshape(
            data.n_covariates,
        )

        comparison_df = pd.DataFrame(
            {
                data.treated_unit: np.round(data.unscaled_treated_covariates.ravel(), 2),
    ...
    def _pre_post_rmspe_ratios(self, placebo_outcomes, placebo=True):
        ...
        if not placebo:
            ...
            rmspe_df = pd.DataFrame(
                {"unit": data.treated_unit, "pre_rmspe": pre_ratio_list, "post_rmspe": post_ratio_list}, columns=["unit", "pre_rmspe", "post_rmspe"]
            )
            ...
            # Store in dataframe
            rmspe_df = pd.DataFrame(
                {"unit": data.control_units, "pre_rmspe": pre_ratio_list, "post_rmspe": post_ratio_list}, columns=["unit", "pre_rmspe", "post_rmspe"]
            )
