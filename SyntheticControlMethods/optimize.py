...
class Optimize(object):
    ...
    def optimize(
        ...
        """
        args = (treated_outcome, treated_covariates, control_outcome, control_covariates, pairwise_difference, pen, placebo, data)
        ...
        for step in range(steps):
            ...
            # Optimze
            res = minimize(self.total_loss, v_0, args=(args), method="L-BFGS-B", bounds=bnds, options={"gtol": 1e-8, "disp": 3, "iprint": 3})
    ...
    def total_loss(self, v_0, treated_outcome, treated_covariates, control_outcome, control_covariates, pairwise_difference, pen, placebo, data):
        """
    ...
    def _update_original_data(self, placebo):
        ...
        if not placebo:
            ...
            self.original_data.synth_constant, self.original_data.synth_outcome = self._get_dsc_outcome(
                self.original_data.w,
            ...
            _, self.original_data.in_time_placebo_outcome = self._get_dsc_outcome(
                self.original_data.in_time_placebo_w,
                self.original_data.control_outcome_all,
