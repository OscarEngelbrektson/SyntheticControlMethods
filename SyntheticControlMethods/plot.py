...
class Plot(object):
    ...
    def plot(
        ...
        valid_panels = ["original", "pointwise", "cumulative", "in-space placebo", "rmspe ratio", "in-time placebo"]
        solo_panels = ["rmspe ratio"]  # plots with different axes
        for panel in panels:
            if panel not in valid_panels:
                raise ValueError(
                    '"{}" is not a valid panel. Valid panels are: {}.'.format(panel, ", ".join(['"{}"'.format(e) for e in valid_panels]))
                )
        ...
        if "rmspe ratio" in panels:
            ...
            ax.barh(y_pos, sorted_rmspe_df["post/pre"], color="#3F5D7D", ec="black")
